...
- **Structured output con `enum`** sobre el esquema cerrado de 6+6 tipos: `response_format: json_schema` con los `enum` tomados de `ENTITY_TYPES`/`RELATION_TYPES`, con caída automática a `json_object` si el proveedor lo rechaza. `temperature: 0.15` — la extracción no es creativa: el mismo guion tiene que dar el mismo grafo en la corrida de evaluación. Prompt y esquema en inglés, con guard de idioma.
- **La key y el endpoint del LLM tienen que ser del mismo proveedor.** Una key de Groq (`gsk_`) contra el endpoint de NVIDIA —o al revés— devuelve **401** en cada lote y todos se descartan. Es el heredero de una trampa anterior: con NVIDIA, `NVIDIA_LLM_MODEL` sin el prefijo `meta/` daba **404** en cada lote y no se vio, porque el cliente hacía `if (!response.ok) return undefined` sin una línea de log. Ahora `llm.ts` reporta los tres primeros fallos por consola con el estado y el cuerpo, y `GET /api/extractor/report` lo avisa en `providerMatches`. Un fallo del LLM no puede romper la sala, pero tampoco puede ser mudo.
- **En Groq, `llama-3.3-70b-versatile` no admite `json_schema`** — devuelve 400 y hay que caer a `json_object`. Ojo al discriminar: los modelos de razonamiento (`openai/gpt-oss-*`) devuelven **otro** 400, `json_validate_failed`, cuando el razonamiento se come el presupuesto de tokens y el JSON sale cortado. El primero es permanente y el segundo es un lote malo; tratarlos igual apaga el structured output de toda la sesión por un fallo pasajero (`rejectsJsonSchema` en `llm.ts`).
- **Groq limita por tokens por minuto** (12.000 en el 70b), no por peticiones. El prompt crece con la lista de nodos, así que una sesión larga topa: el 429 descarta el lote y lo arrastra según la política del 08. Medido en `internal/probe-groq.mts`.
- **Los turnos van al prompt con el nombre de quien habló**, resuelto desde la presencia del canal del agente (`resolveSpeaker`, inyectado en el pipeline para que siga siendo puro). Antes se numeraban `Speaker 1..N` por posición en la ventana, así que la misma persona cambiaba de etiqueta entre lotes y `PROPOSED_BY` —que es un tipo del esquema y una fila del gold— no tenía a quién apuntar.
...
- **Qué carril de Portal entrega y cuál no.** Medido con dos clientes headless contra el Portal real (`@portalsdk/core` 0.1.5, la última publicada). No deducirlo otra vez; no volver a intentar los dos primeros:

  | carril | ¿llega? | evidencia |
  |---|---|---|
  | efímero (`send({ ephemeral: true })`) | **no** | `MessageBuffer.ingest` hace `if (msg.seq === null || msg.ephemeral) continue` **antes** de emitir `message`. 10 enviados → 0 recibidos |
  | metadata de presencia (`setMetadata`) | **no a mitad de sesión** | el protocolo promete que un `meta` «se re-anuncia por presence deltas»; el servidor no lo hace. 10 `setMetadata` → 0 eventos de presencia en el otro cliente. Sí viaja en la trama de conexión, así que sirve de late-join |
  | actividad (`sendActivity`) | **sí** | 40 enviados a 8 Hz → 40 recibidos, hueco medio 138 ms, conexión estable. El `kind` es texto libre y expira solo a los 5 s |
  | persistente (`send`) | sí | es como viaja `graph.delta`, que llega con `ephemeral: false` |

  El efímero era la causa raíz de dos síntomas que parecían distintos —cursores quietos y banners del agente que no salían nunca— y los dos se mudaron a **actividad**.
- **Render**: `d3-force` **anclado** a la siembra determinista de `seedPosition` (`forceX`/`forceY` flojos hacia la posición que decide el `id`, más link/charge/collide). El ancla es lo que sostiene la historia 29 —que las tres pantallas vean el mismo grafo— porque es lo único del layout que no depende del orden de llegada; la simulación solo ordena alrededor. Sustituye a `relaxNeighbors`, que se borró. Zoom y paneo con `d3-zoom`; el radio del nodo crece con la **raíz** de su grado y con techo (`lib/graphView.ts`). Arrastre puramente local, no sincronizado. Aristas `SUPPORTS` verdes / `CONTRADICTS` rojas.
- **Cursores por actividad**: la posición viaja **dentro del `kind`** (`xy|123|456`, `encodeCursorActivity`), throttled a 125 ms. Es feo y es deliberado: los dos carriles que servirían para esto están medidos y muertos (tabla de arriba), y el `kind` es el único campo de texto libre que llega. Las ocho muestras por segundo se convierten en movimiento continuo con la interpolación por `requestAnimationFrame` de `CursorLayer`, que vive en su propio componente para no repintar los cuarenta nodos en cada fotograma. `setMetadata({ cursor })` se sigue escribiendo cada 2 s, solo como late-join. Las coordenadas van en el **espacio del mundo** del grafo, así que «señalo este nodo» significa lo mismo con el zoom de cada uno.
- **Estado del agente por actividad**: `sendActivity("thinking"/"skipped")` en vez de efímeros. Además de que sí se entrega, expira sola a los 5 s — que es justo lo que el ticket 08 le pedía al banner y antes se hacía con un `setTimeout` en el cliente.
- **Next lee los `.env` de `apps/web`, no los de la raíz del monorepo.** `next.config.mjs` carga el `.env` de la raíz a mano para que haya un solo sitio donde poner las claves. Sin eso, `NEXT_GROQ_API_KEY` y `PORTAL_WEBHOOK_SECRET` llegaban vacías y no aparecía **ni un nodo**: el extractor se construye en su versión que siempre falla y el webhook rechaza cada POST, las dos rutas en silencio porque están escritas para no romper la sala. `GET /api/extractor/report` responde `listo: true/false` con qué clave falta; es el primer sitio donde mirar cuando el grafo no se forma.
- **Documento final**: plantilla determinista que recorre el grafo entero — "Ideas de la conversación" (los nodos, Claim/Concept/…), "Relaciones" (cada arista como frase), decisiones con su cadena de soporte, contradicciones sin resolver y `Question` sin `ANSWERS`. Sin la sección de ideas, un grafo de Claims/Concepts salía en el markdown casi vacío y parecía "cualquier cosa". La síntesis por modelo grande se dispara al cerrar y **no bloquea**: el markdown se muestra completo de inmediato y el párrafo se inserta arriba cuando llega.
...
- **Seam aprobado 1 — `graph-core`** (`packages/graph-core/src/merge.ts`, `src/delta.ts`, `src/guards.ts` y `src/document.ts`): tests de comportamiento externo (grafo+propuesta → grafo/delta/markdown), sin servidor, sin Portal, sin red, sin reloj. `renderDocument(graph) -> string` es la plantilla determinista del documento final: lista los nodos como ideas, las aristas como relaciones, decisiones con su cadena de soporte (SUPPORTS/ELABORATES transitivos), contradicciones CONTRADICTS y `Question` sin `ANSWERS`. Los nombres describen la regla de negocio, no el método.
...
- **Seam aprobado 3 — layout del grafo** (`apps/web/lib/layout.ts`, `apps/web/lib/graphView.ts`): siembra determinista de posición desde el `id` del nodo (`seedPosition`, función pura: misma posición para el mismo `id` en cualquier pantalla, dentro del lienzo), que es el ancla de la simulación. Y las reglas del render que sí se pueden equivocar en silencio: `degrees` cuenta la arista por sus dos extremos, `nodeRadius` crece con la raíz del grado y con techo —un nodo sin tope se come la pantalla—, `neighborhood` es el conjunto que se ilumina al posar el puntero. Lógica pura sobre el contrato de `graph-core`, sin importar el SDK.
- **Seam aprobado 4 — cursores en vivo** (`apps/web/lib/cursor.ts`): throttle del envío (`shouldEmitCursor` decide según el último envío), lectura de la posición desde la metadata de presencia (`cursorFromMetadata`, que desde el descubrimiento del carril efímero es el transporte de verdad), fusión de las dos fuentes (`mergeRemoteCursors`) e interpolación entre muestras (`stepTowards`, con aterrizaje exacto para que el bucle pueda pararse). Atribución con `resolveDisplayName`. Lógica pura sobre datos con forma del SDK, sin importar el SDK.
- **Seam aprobado 7 — anchos de los paneles** (`apps/web/lib/panels.ts`): `clampWidth` y `widthFromDragX`. Cubre las dos cosas que no pueden fallar cuando el ancho lo decide un arrastre: que no se coma el lienzo, y que un valor guardado de otra versión no deje la pantalla inservible al volver.
- **Seam aprobado 6 — extractor** (`apps/web/lib/extract/`): el filtro de la primera línea del webhook (`filter.ts`, namespace + `channelId`, corta el bucle de auto-alimentación), el buffer de turnos con debounce (`buffer.ts`) y el `buildPrompt` (`prompt.ts`, ventana de 8 turnos con el nombre de quien habló + lista completa de nodos del espejo + esquema 6+6 tomado de `ENTITY_TYPES`/`RELATION_TYPES` + guard de idioma). El pipeline (`pipeline.ts`) cierra el lote al debounce y **arrastra los turnos a la ventana siguiente si el LLM o la entrega fallan**. Lógica pura, sin LLM ni Portal; el cliente LLM (`llm.ts`) y el runtime (`runtime.ts`) son transporte y no se testean.
