...
- **Seam aprobado 1 — `graph-core`** (`packages/graph-core/src/merge.ts` y `src/document.ts`): tests de comportamiento externo (grafo+propuesta → grafo/delta/markdown), sin servidor, sin Portal, sin red, sin reloj. `renderDocument(graph) -> string` es la plantilla determinista del documento final: decisiones con su cadena de soporte (SUPPORTS/ELABORATES transitivos), contradicciones CONTRADICTS y `Question` sin `ANSWERS`. Los nombres describen la regla de negocio, no el método.
