...
- **El modelo de NVIDIA lleva namespace.** `NVIDIA_LLM_MODEL` tiene que ser `meta/llama-3.1-8b-instruct`; sin el prefijo el endpoint devuelve **404** y todos los lotes se descartan. Pasó y no se vio porque el cliente hacía `if (!response.ok) return undefined` sin una línea de log; ahora `nvidia.ts` reporta los tres primeros fallos por consola con el estado y el cuerpo. Un fallo del LLM no puede romper la sala, pero tampoco puede ser mudo.
