...
- [ ] La extensión está desplegada y su `onBatch` recibe mensajes del namespace `graph.`
- [ ] `onBatch` emite el delta como **valor de retorno**, no como un publish separado
- [ ] El cliente se suscribe a `graph.delta` y pinta los nodos y aristas que llegan
- [ ] Disparado con un `send()` desde la consola de una pestaña, el mismo nodo aparece en las tres
- [ ] El badge de versión del grafo refleja la versión del último delta recibido
- [ ] Queda anotado en `spike.md` cuánto dura el ciclo editar→desplegar→probar de una extensión (X1-Q4)
- [ ] Queda comunicado al equipo que el contrato de datos está congelado: cambiarlo a partir de aquí requiere acuerdo de los tres
